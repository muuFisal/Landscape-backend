<div>
    <form class="form form-horizontal" wire:submit.prevent='submit'>
        <div class="border rounded p-2 mb-3">
            <h5 class="mb-2 text-primary text-capitalize">{{ __('dashboard.work-page-details') }}</h5>
            
            <div class="row">
                <div class="mb-1 col-md-6">
                    <label class="form-label">{{ __('dashboard.eyebrow-ar') }}</label>
                    <input type="text" class="form-control" wire:model.defer="eyebrow_ar">
                    @include('dashboard.includes.error', ['property' => 'eyebrow_ar'])
                </div>
                <div class="mb-1 col-md-6">
                    <label class="form-label">{{ __('dashboard.eyebrow-en') }}</label>
                    <input type="text" class="form-control" wire:model.defer="eyebrow_en">
                    @include('dashboard.includes.error', ['property' => 'eyebrow_en'])
                </div>
            </div>
            ...
            <div class="mb-1">
                ...
                <div class="mb-2 p-1 border rounded bg-light-subtle">
                    @if ($image instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile)
                        <img src="{{ $image->temporaryUrl() }}" width="180" class="img-fluid rounded">
                    @elseif(!empty($image))
                        <img src="{{ asset($image) }}" width="180" class="img-fluid rounded">
                    @endif
                </div>
        ...
        <button type="submit" class="btn btn-primary" wire:loading.attr="disabled">
            <span wire:loading.remove>{{ __('dashboard.submit') }}</span>
            <span wire:loading><i class="fa fa-spinner fa-spin"></i> {{ __('dashboard.wait') }}</span>
        </button>
    </form>
</div>
